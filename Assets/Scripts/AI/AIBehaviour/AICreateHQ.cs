    ...
    public override void Execute()
    {
        ...
        else //Can not build at this place
        {
            foreach (GameObject unit in support.Builders) //try to find a new building site for the next frame
            {
                ...
                float Corner1PosX = CamController.instance.Corner1.position.x;
                float Corner2PosX = CamController.instance.Corner2.position.x;
                float Corner1PosZ = CamController.instance.Corner1.position.z;
                float Corner2PosZ = CamController.instance.Corner2.position.z;
